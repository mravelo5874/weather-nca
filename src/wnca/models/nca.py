...
class ForecastModel(nn.Module):
    ...
    def forecast_step(self, state, static, prev_phys=None, z=None):  # pragma: no cover
    ...
    def _cond(self, state, static, prev_phys):
        if not self.cfg.state.second_order:
            return static
    ...
    def rollout(self, state, static, n_windows, prev_phys=None, z=None, return_aux=False):
        ...
        for _ in range(n_windows):
            state = self.forecast_step(state, static, prev_phys, z)
    ...
    def rollout_ensemble(self, state, static, n_windows, prev_phys=None, n_members=1,
                         z=None, generator=None, return_aux=False):
